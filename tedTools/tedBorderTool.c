...
static void tedBorderToolShowWide(	BorderTool *		bt )
	...
	{
	guiLengthToText( bt->btWideText,
					ebp->ebpPenWideTwips, UNITtyPOINTS );
	}
    else{ guiStringToText( bt->btWideText, "" );	}
...
static void tedBorderToolSetProperties(	BorderTool *			bt,
    ...
    guiSetToggleState( bt->btOnOffToggle, set );
    tedBorderToolShowWide( bt );

    if  ( bt->btStyleRow )
	{
	guiExposeDrawnPulldownInplace( &(bt->btStylePulldown) );
	}
...
static APP_REDRAW_HANDLER_H( tedBorderToolRedrawInplace, w, voidbt, exposeEvent )
    {
    ...
    guiStartDrawing( &drClip, dsI, bt->btStylePulldown.adpInplaceDrawing,
								exposeEvent );

    guiDrawnPulldownDrawArrow( &drClip, &wide, &high, w,
						&(bt->btStylePulldown) );
...
static APP_REDRAW_HANDLER_H( tedBorderToolRedrawPulldown, w, voidbt, exposeEvent )
    {
    BorderTool *	bt= (BorderTool *)voidbt;
    DrawingSurface	dsP;
    ...
    if  ( guiFinishDrawnPulldownPulldown( &(bt->btStylePulldown) ) )
	{ LDEB(1); return;	}
    ...
    guiStartDrawing( &drClip, dsP, bt->btStylePulldown.adpPulldownDrawing,
								exposeEvent );
...
static APP_EVENT_HANDLER_H( tedBorderToolClickedPulldown, w, voidbt, mouseEvent )
    ...
    /*  1  */
    if  ( guiDrawnPulldownGetStrip( &style,
...
static APP_TXACTIVATE_CALLBACK_H( tedFormatBorderWidthChanged, w, voidbt )
    ...
    if  ( guiLengthFromText( bt->btWideText,
				&wide, &changed, UNITtyPOINTS,
				minValue, adaptToMin, maxValue, adaptToMax ) )
	{ return;	}

    guiLengthToText( bt->btWideText, wide, UNITtyPOINTS );
...
void tedInitBorderTool(		BorderTool *		bt )
    {
    bt->btInspector= (struct AppInspector *)0;
    bt->btSubjectPage= -1;
    bt->btWhich= -1;
    ...
    guiInitDrawnPulldown( &(bt->btStylePulldown) );
...
#   if USE_FRAME
#	if USE_FRAME_TITLE
	guiMakeColumnFrameInColumn( &(bt->btFrame), &(bt->btPaned), pageWidget,
				title );
#	else
	guiMakeColumnFrameInColumn( &(bt->btFrame), &(bt->btPaned), pageWidget,
				(const char *)0 );
...
#   endif
    ...
    /**/
    bt->btOnOffRow= guiMakeRowInColumn( rowParent,
					ONOFF_ROW_COLUMNS, heightResizable );

#   if USE_TOGGLE_TITLE
    bt->btOnOffToggle= guiMakeToggleInRow( bt->btOnOffRow, title,
				tedBorderToolOnOffToggled, (void *)bt,
				ONOFF_COLUMN, ONOFF_COLSPAN );
#   else
    bt->btOnOffToggle= guiMakeToggleInRow( bt->btOnOffRow, (const char *)0,
				tedBorderToolOnOffToggled, (void *)bt,
				ONOFF_COLUMN, ONOFF_COLSPAN );
#   endif

    guiMakeTextInRow( &(bt->btWideText), bt->btOnOffRow,
				THICK_COLUMN, THICK_COLSPAN, THICK_CHARS,
				textEnabled );

    guiSetGotValueCallbackForText( bt->btWideText,
				tedFormatBorderWidthChanged, (void *)bt );
    guiSetTypingCallbackForText( bt->btWideText,
				tedFormatBorderWidthTyped, (void *)bt );

#   if MAKE_COLOR_ROW
    guiMakeLabelAndColorChooserRow( &(bt->btColorRow), &(bt->btColorLabel),
			    &(bt->btColorChooser),
...
#   else
    ...
    guiMakeColorChooserInRow( &(bt->btColorChooser), bordersHaveAutomaticColor,
				    bt->btColorRow,
...
#   endif
    ...
    /**/
#   if 0
    bt->btStyleRow= guiMakeRowInColumn( bt->btPaned, 2, heightResizable );
#   else
    bt->btStyleRow= (APP_WIDGET)0;
#   endif
    ...
    if  ( bt->btStyleRow )
	{
	guiMakeLabelInRow( &(bt->btStyleLabel), bt->btStyleRow,
				labelColumn, labelColspan, btr->btrStyle );

	guiMakeDrawnPulldownInRow( &(bt->btStylePulldown),
...
void tedFinishBorderTool(	BorderTool *			bt,
    ...
    if  ( bt->btStyleRow )
	{
	if  ( guiFinishDrawnPulldownInplace( &(bt->btStylePulldown) ) )
	    { LDEB(1); return;	}

	guiSetDrawnPulldownStrips( &(bt->btStylePulldown), DOCbs_COUNT );
	}
...
int tedBorderToolGetNumber(	int *			pNum,
    ...
    bp= *docGetBorderPropertiesByNumber( bd, *pNum );
