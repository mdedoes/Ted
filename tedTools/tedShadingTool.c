...
static void tedShadingToolShowLevel(	ShadingTool *		st,
	...
	{
	guiIntegerToText( st->stLevelText, is->isLevel/ 100 );
	}
    else{ guiStringToText( st->stLevelText, "" );	}
...
static void tedSetShadingTool(		ShadingTool *			st,
    ...
    appColorChooserSetColor( &(st->stBackColorChooser),
				    ...
				    &(eis->eisBackColor) );
    guiSetToggleState( st->stOnOffToggle, eis->eisBackColorExplicit );
    ...
    if  ( st->stShowPattern )
	{
	guiExposeDrawnPulldownInplace( &(st->stPatternPulldown) );
	}
...
void tedSetShadingToolByNumber(		ShadingTool *			st,
    ...
    is= docGetItemShadingByNumber( bd, num );

    tedSetShadingTool( st, bd->bdProperties, is );
...
static APP_EVENT_HANDLER_H( tedShadingRedrawInplace, w, voidst, exposeEvent )
    ...
    guiDrawnPulldownDrawArrow( &wide, &high, w, &(xx) );
...
static APP_REDRAW_HANDLER_H( tedShadingRedrawPulldown, w, voidst, exposeEvent )
    {
    ShadingTool *	st= (ShadingTool *)voidst;
    ...
    guiStartDrawing( &drClip, st->stPulldownDrawingDataSet, add, ox, oy, exposeEvent );
...
static APP_EVENT_HANDLER_H( tedShadingClickedPulldown, w, voidst, mouseEvent )
    ...
    if  ( guiDrawnPulldownGetStrip( &pattern,
...
void tedInitShadingTool(		ShadingTool *		st )
    ...
    st->stShowPattern= 0;
    guiInitDrawnPulldown( &(st->stPatternPulldown) );
    ...
    st->stTarget= (void *)0;
    st->stInspector= (struct AppInspector *)0;
...
void tedCleanShadingTool(		ShadingTool *		st )
    ...
    guiCleanDrawnPulldown( &(st->stPatternPulldown) );
...
void tedFormatMakeShadingTool(	ShadingTool *			st,
    ...
    */
    ...
    if  ( title )
	{
	guiMakeColumnFrameInColumn( &(st->stFrame),
...
#   if SHOW_PATTERN
    ...
    guiMakeLabelInRow( &(st->stPatternLabel), row,
				labelColumn, labelColspan, str->strPattern );
    guiMakeDrawnPulldownInRow( &(st->stPatternPulldown),
...
#   endif
    ...
    if  ( st->stLevelText )
	{
	guiSetTypingCallbackForText( st->stLevelText,
					tedShadingLevelTyped, (void *)st );
	}

    /****/
#   if 0
    guiMakeLabelAndColorChooserRow( &(st->stRow), &(st->stBackLabel),
			    &(st->stBackColorChooser),
			    parent, str->strBackColor,
			    &(str->strBackColorChooserResources),
			    tedShadeColorChosen, backWhich, (void *)st );
#   else
    guiMakeToggleAndColorChooserRow( &(st->stRow), &(st->stOnOffToggle),
...
#   endif
    ...
    /****/
#   if 0
    guiMakeLabelAndColorChooserRow( &(st->stRow2), &(st->stForeLabel),
...
void tedFinishShadingTool(	ShadingTool *			st,
    ...
    if  ( st->stShowPattern )
	{
	if  ( guiFinishDrawnPulldownInplace( &(st->stPatternPulldown) ) )
	    { LDEB(1); return;	}

	guiSetDrawnPulldownStrips( &(st->stPatternPulldown), PSshd_COUNT );
	}
...
static int tedShadingToolGetShading(
	...
	if  ( guiIntegerFromText( st->stLevelText, &level,
...
int tedShadingToolGetShadingNumber(	int *			pNum,
    ...
    is= *docGetItemShadingByNumber( bd, *pNum );
