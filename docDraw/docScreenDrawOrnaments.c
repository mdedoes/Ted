/************************************************************************/
/*									*/
/*  Draw ornaments: Borders and Shading.				*/
/*									*/
/************************************************************************/
...
int docScreenDrawSetColorRgb(	DrawingContext *	dc,
				void *			vsdd,
...
int docScreenDrawSetFont(	DrawingContext *	dc,
				void *			vsdd,
				int			textAttrNr,
...
static void docScreenDrawSetBorderColor( DrawingContext *	dc,
...
int docScreenDrawOrnaments(
			const BlockOrnaments *		bo,
			int				page,
	...
	if  ( ! dc->dcClipRect						||
	      geoIntersectRectangle( &drShade, &drInPixels,
						    dc->dcClipRect )	)
	    {
	    const ItemShading *		is= bo->boShading;
		...
		if  ( bo->boShading->isForeColor == 0 )
		    ...
		    isX.isLevel= 100* level;
		    isX.isPattern= DOCspSOLID;
	...
	if  ( ! dc->dcClipRect						  ||
	    ...
	    {
	    docScreenDrawSetBorderColor( dc, sdd,
...
void docScreenDrawShadedRectangle(
