...
static void fontMatchPsFacesToDocFont(	DocumentFont *		df,
					const PsFontFamily *	psf )
    {
    int		fac;
    int		guessFonstspecific= ! df->dfPsFontInfo[0];
	...
	int	idx= FACE_INDEX(s,b);

	df->dfPsFontInfo[idx]= psf->psfFaces[fac];
	}
    ...
    if  ( guessFonstspecific && df->dfPsFontInfo[0] )
	{
	df->dfFontSpecificEncoding=
			df->dfPsFontInfo[0]->afiFontSpecificEncoding;
	}
...
/*									*/
/*  If we do not know for sure that the font has FontSpecific encoding,	*/
/*  just insert the font as an ANSI font: The exact details only count	*/
/*  once the font has been used and the document is saved.		*/
/*									*/
/************************************************************************/

int fontAddPsFontsToDocList(	struct DocumentFontList *	dfl,
	...
	df= fontAddDocFontToList( dfl, aff->psfFontFamilyName,
							styleInt, pitch );
	if  ( ! df )
	    { SXDEB(aff->psfFontFamilyName,df); return -1;	}

	fontMatchPsFacesToDocFont( df, aff );
	}
...
int fontAddBase35FontsToDocList(	struct DocumentFontList *	dfl )
    {
    int				fam;
	...
	df= fontAddDocFontToList( dfl, b35f->b35fName,
...
const char * fontGetEncodingName(	const MemoryBuffer *	fontName,
					int			charset )
    {
    ...
    /*  1  */
    if  ( utilMemoryBufferEqualsString( fontName, "ITC Zapf Dingbats" )	||
	  utilMemoryBufferEqualsString( fontName, "ZapfDingbats" )	||
	  utilMemoryBufferEqualsString( fontName, "Dingbats" )		)
	{ return "DINGBATS"; }

    /*  3  */
    if  ( utilMemoryBufferEqualsString( fontName, "Symbol" ) )
	{ return "SYMBOL"; }
...
int fontFindLegacyEncodings(	DocumentFont *	df )
    {
    int			rval= 0;
    ...
    if  ( utilMemoryBufferEqualsString( &(df->dfName), "ITC Zapf Dingbats" )	||
	  utilMemoryBufferEqualsString( &(df->dfName), "Symbol" )		)
	{ goto superfluous;	}
	...
	legacySet= textOfficeCharsetGetCodeSet( csIdx );
	if  ( ! legacySet )
	    { continue;	}
  ...
  superfluous:
    ...
    /*  Nothing found.. Take FONTcharsetSYMBOL */
