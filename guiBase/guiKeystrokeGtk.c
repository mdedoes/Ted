...
# define	IS_MODIFIER(k) ( ( (k)>=KEY_Shift_L && (k) <= KEY_Hyper_R ) || \
				 (k) == KEY_Num_Lock )
...
static void guiDocumentCommitText(	GtkIMContext *	im,
					const gchar *	str,
					void *		voiddw )
    {
    DocumentWidget *	dw= (DocumentWidget *)voiddw;
...
void guiDocumentSetInputContext(	APP_INPUT_METHOD	im,
    ...
    g_signal_connect_after( G_OBJECT( dw->dwWidget ), "key_press_event",
				    (GCallback)guiKeyPressed, (void *)dw );
...
#   ifdef GTK_TYPE_IM_CONTEXT
	...
	g_signal_connect( dw->dwInputContext, "commit",
			G_CALLBACK (guiDocumentCommitText), (void *)dw );
#	if 0
