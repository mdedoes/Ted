...
/*  3  */
typedef struct X11FontEncoding
    {
    ...
    XFontFamily *	feXfontFamilies;
    int			feXfontFamilyCount;
    char **		feXFontList;	/* from XListFonts()		*/
    int			feXFontCount;	/* from XListFonts()		*/
    } X11FontEncoding;

static X11FontEncoding	AppFontXEncodings[]=
{
...
static const int AppFontXEncodingCount=
			    sizeof(AppFontXEncodings)/sizeof(X11FontEncoding);
...
static int appFontXCatalog(	Display *		display,
    ...
    appFontQueryString( fontFormat,
		...
		fe->feX11Reg_Enc );	/* registry,encoding	*/

    fe->feXFontList= XListFonts( display, fontFormat,
						4000, &(fe->feXFontCount) );
    if  ( fe->feXFontCount == 4000 )
	{ LLDEB(fe->feXFontCount,4000);	}

    for ( idx= 0; idx < fe->feXFontCount; idx++ )
	{
	char			foundry[40];
	char			family[40];
	...
	if  ( strlen( fe->feXFontList[idx] ) >= X_FONTNAME_MAX )
	    { SDEB(fe->feXFontList[idx]); continue; }

	if  ( appFontSplitXFont( fe->feXFontList[idx],
					foundry,
					family,
					weightStr,
					slantStr,
					...
					ignore,		/*  registry  */
					ignore ) )	/*  encoding  */
	    { LSDEB(idx,fe->feXFontList[idx]); continue;	}

	if  ( strcmp( spacing, "m" ) && strcmp( spacing, "p" ) )
	    {
	    if  ( strcmp( spacing, "c" ) )
		{ SSDEB(spacing,fe->feXFontList[idx]);	}
	...
	if  ( psFontGetWidth( &swdth, swdthStr ) )
	    { SSDEB(swdthStr,fe->feXFontList[idx]); continue;	}

	xff= fe->feXfontFamilies;
	for ( fam= 0; fam < fe->feXfontFamilyCount; xff++, fam++ )
	    {
	...
	if  ( fam >= fe->feXfontFamilyCount )
	    {
	    char *	s;

	    xff= (XFontFamily *)realloc( fe->feXfontFamilies,
			(fe->feXfontFamilyCount+1)* sizeof(XFontFamily) );
	    if  ( ! xff )
		{ LXDEB(fe->feXfontFamilyCount,xff); return -1;	}
	    fe->feXfontFamilies= xff;

	    xff += fe->feXfontFamilyCount;
	...
	if  ( face >= xff->xffFaceCount )
		...
		{ SSDEB(fe->feXFontList[idx],weightStr); continue;	}
	...
	if  ( pxlsz == 0 )
		...
		{
		/*
		SLLSDEB("DUPLICATE",idx,sz,fe->feXFontList[idx]);
		LSDEB(xffs->xffsListIndex,
				fe->feXFontList[xffs->xffsListIndex]);
		*/
		continue;
		}
    ...
    /* NO! XFreeFontNames( fe->feXFontList ); */
...
static int appFontXFontName(	char *			target,
    ...
    if  ( ! afi || ! fe )
	{ XXDEB(afi,fe); return -1;	}

    if  ( fe->feX11Reg_Enc_Strlen == 0 )
	{ fe->feX11Reg_Enc_Strlen= strlen( fe->feX11Reg_Enc );	}

    for ( f= 0; f < afi->afiX11FontCount; f++ )
	{
	int		l11= strlen( afi->afiX11Fonts[f] );
	int		o11= l11- fe->feX11Reg_Enc_Strlen;

	if  ( o11 > 0							&&
	      afi->afiX11Fonts[f][o11-1] == '-'				&&
	      ! strcmp( afi->afiX11Fonts[f]+ o11, fe->feX11Reg_Enc )	)
	    { t1Entry= afi->afiX11Fonts[f]; break;	}
	}

    /*  1  */
    appFontXCatalog( display, fe );

    if  ( ! fe->feXfontFamilies )
	{ return -1;	}
    ...
    best= 0;
    fam= -1;
    for ( f= 0; f < fe->feXfontFamilyCount; xff++, f++ )
	{
	int	score;
	...
	if  ( dv <= 115					||
	    ...
	    if  ( strlen( fe->feXFontList[idx] ) >= X_FONTNAME_MAX )
		{ SDEB(fe->feXFontList[idx]); }

	    strcpy( target, fe->feXFontList[idx] );
	...
	scalableEntry= fe->feXFontList[xft->xftScalableListIndex];
	}
...
static int appFontGetEncodingInfo(	const IndexMapping **	pIm,
	...
	{
	if  ( textSetLegacyMapping( im, is, unicodes )	)
	    { SDEB(fontName); return -1;	}
	}
...
static int appFontX11TryEncoding(	EncodedScreenFont *	sf,
    ...
    if  ( utilMemoryBufferIsEmpty( &(dsf->dsfFontNameX11) ) )
	{
	utilMemoryBufferSetString( &(dsf->dsfFontNameX11), sf->esfFontName );
	}
...
int appFontOpenX11Fonts(	DrawScreenFont *		dsf )
	...
	{
	int	n= utilCopyIndexSet( &isRequired, dsf->dsfUnicodesUsed );
	...
	if  ( n == 0 )
	    {
	    utilIndexSetAddArray( &isRequired,
					uniIso8859_15GlyphUnicodes, 256 );
	    }
	}
    else{
	utilIndexSetAddArray( &isRequired, uniIso8859_15GlyphUnicodes, 256 );
	}
	...
	{
	X11FontEncoding *	fe= AppFontXEncodings;
...
void drawX11GetFontProperties(	EncodedScreenFont *	esf,
...
void drawCleanEncodedScreenFontList(	DrawScreenFont *	dsf,
					EncodedScreenFontList * esfl )
    {
    int			i;
...
void drawInitEncodedScreenFontList(	EncodedScreenFontList * esfl )
    {
