...
static void guiDebugKey(	const char *		how,
...
static void guiHandleKeysym(		DocumentWidget *	dw,
					char *			scratch,
					APP_KEY_VALUE		keysym,
					int			state )
    {
...
static APP_EVENT_HANDLER_H( guiKeyPressed, w, voiddw, event )
    {
	...
	switch( status )
	    ...
	    case XLookupBoth:
		scratch[0]= '\0';
		guiHandleKeysym( dw, scratch, keysym, keyEvent->state );
		if  ( debugKeys > 0 )
		    {
		    guiDebugKey( status==XLookupBoth?"Both":"KeySym",
	...
	guiHandleKeysym( dw, scratch, keysym, keyEvent->state );
	if  ( debugKeys > 0 )
	    { guiDebugKey( ":NoIm", keyEvent, keysym, scratch );	}
...
static XIMStyle	GUIMOTIF_XimPreferences[]=
    {
    XIMPreeditCallbacks,
    XIMPreeditPosition,
    XIMPreeditArea,
...
void guiDocumentSetInputContext(	APP_INPUT_METHOD	im,
					DocumentWidget *	dw )
    {
    XtAddEventHandler( dw->dwWidget, KeyPressMask, False,
						guiKeyPressed, (void *)dw );
	...
	for ( style= 0; style < availableStyles->count_styles; style++ )
	    ...
	    for ( j= 0; j < sizeof(GUIMOTIF_XimPreferences)/sizeof(XIMStyle); j++ )
		{
		if  (   ( foundStyle & GUIMOTIF_XimPreferences[j] )	&&
		      ! ( likedStyle & GUIMOTIF_XimPreferences[j] )	)
