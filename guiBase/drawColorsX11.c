...
int drawColorFindRgb(	XColor *	xc,
			AppColors *	acSys,
			unsigned int	r,
			unsigned int	g,
			unsigned int	b )
...
static AppColors * GUI_MOTIF_TheColors= (AppColors *)0;

AppColors *	drawGetColorsMotif( Display *	display )
    {
    if  ( ! GUI_MOTIF_TheColors )
	{
	GUI_MOTIF_TheColors= malloc( sizeof(AppColors) );
	if  ( ! GUI_MOTIF_TheColors )
	    { XDEB(GUI_MOTIF_TheColors); return (AppColors *)0;	}

	drawInitColors( GUI_MOTIF_TheColors );
	appAllocateColors( display, GUI_MOTIF_TheColors );
	}

    return GUI_MOTIF_TheColors;
    }
