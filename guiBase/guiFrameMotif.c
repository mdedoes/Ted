...
static void guiMakeFrameTitleWidget(	Widget		frame,
					const char *	title )
    {
    Arg		al[20];
    int		ac= 0;
...
static void guiMakeFrame(	Widget *	pFrame,
				Widget		parent,
				const char *	title )
    {
    Arg			al[20];
    ...
    if  ( title )
	{ guiMakeFrameTitleWidget( frame, title );	}
...
void guiMakeColumnFrameInColumn(	APP_WIDGET *		pFrame,
    ...
    guiMakeFrame( &frame, parent, title );
...
int guiSetFrameTitle(		APP_WIDGET		frame,
    ...
    guiMakeFrameTitleWidget( frame, (const char *)title );
