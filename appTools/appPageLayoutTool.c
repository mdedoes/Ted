...
static void appPageLayoutRefreshPageLayoutPage(	PageLayoutTool *	plt )
	...
	{
	guiLengthToText( plt->pltHeaderPositionText,
				dg->dgHeaderPositionTwips, ea->eaUnitInt );
	}

    if  ( plt->pltFooterPositionText )
	{
	guiLengthToText( plt->pltFooterPositionText,
...
static void appPageLayoutChangeLayout(	PageLayoutTool *	plt,
    ...
    {
    EditApplication *		ea= plt->pltApplication;
    struct EditDocument *	ed= ea->eaCurrentDocument;
	...
	if  ( guiLengthFromText( plt->pltHeaderPositionText,
			    &(dgNew.dgHeaderPositionTwips),
			    &changed, ea->eaUnitInt,
			    minValue, adaptToMin, maxValue, adaptToMax ) )
	    { return;	}

	if  ( guiLengthFromText( plt->pltFooterPositionText,
...
static void appSectHeaderFooterPosChanged(	PageLayoutTool *	plt,
    ...
    if  ( ! guiLengthFromText( w, &width, &changed, ea->eaUnitInt,
				minValue, adaptToMin, maxValue, adaptToMax ) )
	{
	guiLengthToText( w, width, ea->eaUnitInt );
	}
    }
...
static APP_REDRAW_HANDLER_H( appPageLayoutToolDrawPage, w, voidplt, exposeEvent )
    {
    PageLayoutTool *		plt= (PageLayoutTool *)voidplt;
...
static void appPageLayoutMakeHeaderFooterWidgets(
    ...
    guiMakeRowFrameInColumn( &(plt->pltHeaderFooterFrame),
					&(plt->pltHeaderFooterInside),
					parent, 2, plpr->plprPositionsText );

    guiMakeColumnInRow( &(plt->pltHeaderFooterLeftColumn),
				    plt->pltHeaderFooterInside, 0, colspan );

    guiMakeColumnInRow( &(plt->pltHeaderFooterRightColumn),
...
#   define	TW	7
    ...
    guiSetGotValueCallbackForText( plt->pltHeaderPositionText,
				appSectHeaderPositionChanged, (void *)plt );

    guiSetGotValueCallbackForText( plt->pltFooterPositionText,
...
void appPageLayoutPageFillPage(	PageLayoutTool *		plt,
    ...
    /**/
    plt->pltInspector= ai;
    plt->pltApplication= ea;

    plt->pltUnitType= geoUnitTypeInt( plpr->plprUnitType );
...
void appInitPageLayoutTool(	PageLayoutTool *	plt )
    {
    memset( plt, 0, sizeof(PageLayoutTool) );
    }
