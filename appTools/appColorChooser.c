...
void appColorChooserSetColor(	ColorChooser *		cc,
    ...
    if  ( changed )
	{
	guiExposeDrawnPulldownInplace( &(cc->ccPulldown) );
	}
...
static APP_REDRAW_HANDLER_H( appColorChooserRedrawInplace, w, voidcc, exposeEvent )
    {
    ...
    guiStartDrawing( &drClip, dsI, cc->ccPulldown.adpInplaceDrawing,
								exposeEvent );

    /* Also draws background */
    guiDrawnPulldownDrawArrow( &drClip, &wide, &high,
						w, &(cc->ccPulldown) );
...
static int appColorChooserAllocatePullDownDrawingData(
    ...
    if  ( guiFinishDrawnPulldownPulldown( adp ) )
	{ LDEB(1); return -1;	}
...
static APP_REDRAW_HANDLER_H( appColorChooserRedrawPulldown, w, voidcc, exposeEvent )
    {
    ColorChooser *	cc= (ColorChooser *)voidcc;
    DrawingSurface	dsP;
    ...
    guiStartDrawing( &drClip, dsP, cc->ccPulldown.adpPulldownDrawing,
								exposeEvent );
...
static APP_EVENT_HANDLER_H( appColorChooserClickedPulldown, w, voidcc, mouseEvent )
    ...
    /*  1  */
    if  ( guiDrawnPulldownGetStrip( &row,
...
void appCleanColorChooser(		ColorChooser *		cc )
    {
    guiCleanDrawnPulldown( &(cc->ccPulldown) );
...
#   if USE_GTK /* Crashes with Motif */
    if  ( guiFinishDrawnPulldownPulldown( &(cc->ccPulldown) ) )
	{ LDEB(1);	}
#   endif

    if  ( guiFinishDrawnPulldownInplace( &(cc->ccPulldown) ) )
	{ LDEB(1);	}
    ...
    guiSetDrawnPulldownStrips( &(cc->ccPulldown), cc->ccStrips );
...
void appInitColorChooser(	ColorChooser *			cc )
    ...
    guiInitDrawnPulldown( &(cc->ccPulldown) );
...
void guiMakeColorChooserInRow(	ColorChooser *			cc,
				int				hasAutomatic,
				APP_WIDGET			row,
				int				col,
				int				colspan,
...
#   endif
    ...
    guiMakeDrawnPulldownInRow( &(cc->ccPulldown),
...
void guiMakeLabelAndColorChooserRow(
				APP_WIDGET *			pRow,
				APP_WIDGET *			pLabel,
				ColorChooser *			cc,
				int				hasAutomatic,
    ...
    guiMakeColorChooserInRow( cc, hasAutomatic, row, ccColumn, ccColspan,
...
void guiMakeToggleAndColorChooserRow(
				APP_WIDGET *			pRow,
				APP_WIDGET *			pToggle,
				ColorChooser *			cc,
				int				hasAutomatic,
...
void appColorChooserColorChosen(struct PropertyMask *		isSetMask,
				int *			pChanged,
				RGB8Color *		rgb8To,
				int *			pExplicit,
...
void appEnableColorChooser(	ColorChooser *		cc,
	...
	guiEnableDrawnPulldown( &(cc->ccPulldown),
