...
void appDrawPageDiagram(	APP_WIDGET			w,
    ...
    drText.drX0= wide/2- pixelsPerTwip*
		    ( dg->dgPageWideTwips/2- dg->dgMargins.roLeftOffset );
    drText.drX1= wide/2+ pixelsPerTwip*
		    ( dg->dgPageWideTwips/2- dg->dgMargins.roRightOffset );
    drText.drY0= high/2- pixelsPerTwip*
		    ( dg->dgPageHighTwips/2- dg->dgMargins.roTopOffset );
    drText.drY1= high/2+ pixelsPerTwip*
		    ( dg->dgPageHighTwips/2- dg->dgMargins.roBottomOffset );
...
void appDrawNupDiagram(	APP_WIDGET			w,
	...
	y0= AT2_Y( dgPage->dgMargins.roLeftOffset,
				    dgPage->dgMargins.roTopOffset, &atPage )+ 0.49;

	x1= AT2_X( dgPage->dgPageWideTwips- dgPage->dgMargins.roRightOffset,
			dgPage->dgPageHighTwips- dgPage->dgMargins.roBottomOffset,
			&atPage )+ 0.49;
	y1= AT2_Y( dgPage->dgPageWideTwips- dgPage->dgMargins.roRightOffset,
			dgPage->dgPageHighTwips- dgPage->dgMargins.roBottomOffset,
			&atPage )+ 0.49;
...
APP_WIDGET appMakePageDrawing(	APP_WIDGET			parent,
    ...
    guiMakeDrawingAreaInColumn( &drawing, parent,
