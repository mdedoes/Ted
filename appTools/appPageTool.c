...
static APP_REDRAW_HANDLER_H( appPageToolDrawPage, w, voidapt, exposeEvent )
    {
...
static APP_BUTTON_CALLBACK_H( appPageToolRevertPushed, w, voidapt )
    ...
    apt->aptGeometryChosen.dgMargins= apt->aptGeometrySet.dgMargins;
...
static APP_TXACTIVATE_CALLBACK_H( appPageToolMarginChanged, w, voidapt )
    ...
    apt->aptGeometryChosen.dgMargins= dg.dgMargins;
...
static APP_WIDGET appPageToolMakeButtonRow( APP_WIDGET		parent,
    ...
    guiMakeButtonInRow( &revertButton, row, aptr->aptrRevert,
	    appPageToolRevertPushed, (void *)apt, 0, colspan, buttonIsDefault );

    guiMakeButtonInRow( &setButton, row, aptr->aptrSet,
...
void * appMakePageTool(	EditApplication *	ea,
    ...
    apt->aptUnitType= geoUnitTypeInt( aptr.aptrUnitType );
