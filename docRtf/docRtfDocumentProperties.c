...
int docRtfRememberDocProperty(	const RtfControlWord *	rcw,
	...
	/*  Document */
	if  ( docSetDocumentProperty( &(rrc->rrcDocument->bdProperties),
						    rcw->rcwID, arg ) < 0 )
	    { SLDEB(rcw->rcwWord,arg); return 0;	}
	}
...
int docRtfCommitDocPropText(	const RtfControlWord *	rcw,
	...
	if  ( docSetDocumentPropertyString( &(rrc->rrcDocument->bdProperties),
						    rcw->rcwID, text, size ) )
	    { SDEB(rcw->rcwWord); rval= -1; goto ready;	}
	}
...
int docRtfDocTimeGroup(	const RtfControlWord *	rcw,
	...
	if  ( docSetDocumentPropertyTime( &(rrc->rrcDocument->bdProperties),
						rcw->rcwID, &(rrc->rrcTm) ) )
	    { SDEB(rcw->rcwWord); return -1;	}
	}
