...
int docRtfReadDocumentTree(	const RtfControlWord *	rcw,
    ...
    docRtfResetTextAttribute( &internRrs, rrc->rrcDocument );
    ...
    /*  2  */
    if  ( ! dt->dtRoot							&&
	  docMakeDocumentTree( rrc->rrcDocument, dt,
	...
	{
	if  ( ignoreEmpty )
	    { docEraseDocumentTree( rrc->rrcDocument, dt );	}
	else{
	    const int	textAttributeNumber= 0;

	    if  ( ! docInsertEmptyParagraph( rrc->rrcDocument, dt->dtRoot,
						textAttributeNumber ) )
	...
	if  ( docParaStrlen( dp.dpNode ) ==  0		&&
	    ...
	    docDeleteNode( rrc->rrcDocument, dt, bi );
	    }
	}
    }
...
int docRtfReadExtTree(		const RtfControlWord *	rcw,
	...
	case DOCinRIGHT_FOOTER:
	    ...
	    sectBi= docRtfGetSectNode( rrc );
	    if  ( ! sectBi )
		{ XDEB(sectBi); return -1;	}

	    dt= docSectionHeaderFooter( sectBi, (unsigned char *)0,
			    &(rrc->rrcDocument->bdProperties), rcw->rcwID );
	    if  ( ! dt )
...
int docRtfReadFootnote(		const RtfControlWord *	rcw,
    ...
    ownerBi= docRtfGetParaNode( rrc );
    if  ( ! ownerBi )
	{ XDEB(ownerBi); rval= -1; goto ready;	}
    sectBi= docGetSectNode( ownerBi );
    if  ( ! sectBi )
	...
	if  ( ownerBi->biParaParticules[part0   ].tpKind == DOCkindSPAN	&&
	      ownerBi->biParaParticules[part0+ 1].tpKind == DOCkindFIELDHEAD )
	    {
	    ...
	    ownerBi->biParaParticules[part0   ].tpKind= DOCkindFIELDHEAD;
