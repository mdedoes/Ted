...
DocumentField *	docClaimFieldCopy(	DocumentFieldList *	dfl,
    ...
    dfTo= docFieldListClaimField( dfl );
    if  ( ! dfTo )
...
DocumentField * docFieldListGetByNumber( const DocumentFieldList *	dfl,
					int				n )
    {
...
int docGetFieldKindByNumber(	const DocumentFieldList *	dfl,
				int				n )
    {
    DocumentField *	df= docFieldListGetByNumber( dfl, n );
...
void docDeleteFieldFromList(	DocumentFieldList *		dfl,
				DocumentField *			df )
    {
    if  ( docFieldListGetByNumber( dfl, df->dfFieldNumber ) != df )
...
DocumentField * docFieldListClaimField(	DocumentFieldList *	dfl )
    {
    void *		vdf;
    int			n;
    DocumentField *	df;
...
int docFieldListFindBookmark(	DocumentField **		pDf,
...
int docFieldListForAll(	const DocumentFieldList *	dfl,
			FieldListForAllFun		fun,
			void *				through )
    {
    return utilPagedListForAll( &(dfl->dflPagedList), (PagedListForAllFun)fun, through );
    }
