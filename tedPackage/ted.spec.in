...
Source:         ftp://ftp.nluug.nl/pub/editors/ted/ted-2.23nx.src.tar.gz
...
%prep
rm -rf Ted-2.23nx
zcat ${RPM_SOURCE_DIR}/ted-2.23nx.src.tar.gz | tar xvf -
##
##  Build section:
##
%build
( cd Ted-2.23nx && make CONFIGURE_OPTIONS=--with-GTK )
( cd Ted-2.23nx && make package CONFIGURE_OPTIONS=--with-GTK )
##
##  Install section:
##
%install
...
cat Ted-2.23nx/tedPackage/ted-2.23nx-linux-*.tar.gz | ( cd %{buildroot} && umask 0 && tar xfz - )
##
##  Files section:
##
%files
...
%attr(755,root,root) /usr/share/Ted/examples/postinstall.sh
...
%attr(644,root,root) /usr/share/Ted/doc/TedDocument-en_US.rtf
%attr(644,root,root) /usr/share/Ted/doc/TedDocument-fr_FR.rtf
%attr(644,root,root) /usr/share/Ted/doc/TedDocument-de_DE.rtf
%attr(644,root,root) /usr/share/Ted/doc/TedDocument-mg_MG.rtf
...
%attr(644,root,root) /usr/share/Ted/ps/pdfautoprint.ps
...
%attr(644,root,root) /usr/share/Ted/Ted48.png
%attr(644,root,root) /usr/share/icons/hicolor/48x48/apps/Ted.png
...
%attr(644,root,root) /usr/share/Ted/varia/ted-2.23nx.lsm
...
%attr(644,root,root) /usr/share/Ted/config/Ted-cs_CZ.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-da_DK.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-de_DE.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-pt_BR.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-fr_FR.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-hu_HU.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-mg_MG.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-oc_FR.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-eu_FR.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-it_IT.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-nl_NL.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-sk_SK.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-nb_NO.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-pl_PL.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-es_ES.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-es_AR.properties
%attr(644,root,root) /usr/share/Ted/config/Ted-fi_FI.properties
##
##  Cleanup section:
##
%clean
( cd Ted-2.23nx && make clean )
rm -rf %{buildroot}/usr/share/Ted
%post
...
update-desktop-database -q || true
%changelog
* Sun Mar 09 2014 Mark de Does <mark@mdedoes.com> 2.23e-1
- New upstream release.
See http://ftp.nluug.nl/pub/editors/ted/announce.html
- Use Enchant (http://http://www.abisource.com/projects/enchant/) 
for spell checking rather than a self-invented wheel.
- Include translations in the general package and eliminate the 
localized packages
- Remember the language inside the document and support multiple languages
inside the same document.
- Make it possible to exclude parts of the text from spell checking.
- Spelling/Language part brushed up.
- Command line calls to concatinate (RTF and other) documents.
- New Footnotes/Endnotes derive formatting from the previous one.
- Formula fields.
- Use much less memory in some cases.
- New tables now have borders around the cells.
- Preparations for GTK3.
- Fixup images without size information.
