...
static int sioLzwSeek(		void *			voidsos,
				long			pos )
    { LDEB(1); return -1;	}
...
SimpleInputStream * sioInLzwGifOpen(	SimpleInputStream *	sisLzw,
    ...
    sis= sioInOpen( (void *)lis, sioInLzwReadBytes,
					sioLzwSeek, sioInLzwClose );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Copied from the ImageMagick source and adapted to fit into the conventions 
%  of this file.
%
...
%  binary LZW bytes are usually encoded as ASCII base-85.
%
...
typedef struct LzwPsOutputStream
    {
    StringTable			losStringTable[4096];
    LzwState			losState;
    SimpleOutputStream *	losSosLzw;
    } LzwPsOutputStream;
...
/************************************************************************/
/*									*/
/*  String table to look for previously known strings.			*/
/*									*/
/*  Initially it contains all strings of length 1: The values in the	*/
/*  original alphabet.							*/
/*									*/
/*  When a string is extended, we append a new item to the end of the	*/
/*  table.								*/
/*									*/
/*  Every item has a 'next' pointer to somewhere else in the array.	*/
/*  ( or -1 if there is no next. ) Every time a possible continuation	*/
/*  after a string (=code) is stored in the table, it is inserted at	*/
/*  the head of the linked list of 'next' pointers from this string.	*/
/*									*/
/*  In this way, the 'next' pointers are a linked list of continuations	*/
/*  of the string that belongs to the current code. The head of the	*/
/*  list is the 'next' of the current code.				*/
/*									*/
/*  TODO: Make the following true:					*/
/*  Though the linked lists are longer than that, we can stop as soon	*/
/*  as soon as the prefix no longer equals the current code: after	*/
/*  that, we will no longer look at direct successors of the current	*/
/*  code, but at indirect ones.						*/
/*									*/
/************************************************************************/

static void sioLzwResetStringTable(	StringTable *	table )
    {
    int		i;
...
static int sioLzwFindNextInStringTable(	StringTable *	table,
					int		currentCode,
					int		val )
    {
    int		idx= table[currentCode].next;

    while( idx >= 0 )
	{
	/*  Try later! Makes it a tree!
	if  ( table[idx].prefix != currentCode )
	    { break;	}
	*/

	if  ( table[idx].prefix == currentCode	&&
	      table[idx].suffix == val		)
	    { return idx;	}
...
static void sioLzwAddNextToStringTable(	StringTable *	table,
					int		currentCode,
					int		nextCode,
					int		val )
    {
    table[nextCode].prefix= currentCode;
    table[nextCode].suffix= val;
    table[nextCode].next= table[currentCode].next;
...
static int sioOutLzwPsWriteCode(	LzwPsOutputStream *	los,
					int			code )
    {
    LzwState *			ls= &(los->losState);
    int				rval= 0;
...
#   if 0
    if  ( code >= ls->lsMaxCode1 )
	{ LLDEB(code,ls->lsMaxCode1); }
    if  ( code>=ls->lsNextCode )
	{ LLDEB(code,ls->lsNextCode); }
#   endif

    shift= 32- ls->lsCurrentCodeSize- ls->lsBitsAccumulated;

    ls->lsAccumulator	  |= ( (unsigned long) code ) << shift;
    ls->lsBitsAccumulated += ls->lsCurrentCodeSize;

    while( ls->lsBitsAccumulated >= 8 )
	{
        if  ( sioOutPutCharacter( ( ls->lsAccumulator  >> 24 ) & 0xff,
							    los->losSosLzw ) )
	    { rval= -1;	}
...
static int sioOutLzwPsClose(	void *		voidlos )
    {
    int				rval= 0;
    LzwPsOutputStream *		los= (LzwPsOutputStream *)voidlos;
    LzwState *			ls= &(los->losState);

    /*  1  */
    if  ( sioOutLzwPsWriteCode( los, ls->lsCurrentCode ) )
	{ XDEB(ls->lsCurrentCode); rval= -1;	}

    /*  2  */
    if  ( sioOutLzwPsWriteCode( los, ls->lsEOFCode ) )
	{ XDEB(ls->lsEOFCode); rval= -1;	}

    /*  3  */
    /* while? */
    if  ( ls->lsBitsAccumulated > 0 )
	{
	if  ( sioOutPutCharacter( ls->lsAccumulator  >> 24,
						    los->losSosLzw ) == EOF )
	    { rval= -1;	}
...
static int sioOutLzwPsWriteBytes(	void *			voidlos,
					const unsigned char *	buffer,
					int			count )
    {
    LzwPsOutputStream *		los= (LzwPsOutputStream *)voidlos;
	...
	nextCode= sioLzwFindNextInStringTable( los->losStringTable,
						    ls->lsCurrentCode, val );
	...
	if  ( nextCode >= 0 )
	    ...
	    sioOutLzwPsWriteCode( los, ls->lsCurrentCode );

	    currentCode= ls->lsCurrentCode;
	    ls->lsCurrentCode= val;
		...
		if  ( sioOutLzwPsWriteCode( los, ls->lsClearCode ) )
		    { LDEB(ls->lsCurrentCode); return -1;	}
		...
		sioLzwResetStringTable( los->losStringTable );
		}
	    else{
		sioLzwAddNextToStringTable( los->losStringTable,
					currentCode, ls->lsNextCode, val );
...
SimpleOutputStream * sioOutLzwPsOpen(	SimpleOutputStream *	sosLzw )
    {
    SimpleOutputStream *	sos;
    LzwPsOutputStream *		los;

    const int			codeSize= 8;

    /*  1  */
    los= (LzwPsOutputStream *)malloc( sizeof(LzwPsOutputStream) );
    if  ( ! los )
	{ XDEB(los); return (SimpleOutputStream *)0;	}
    ...
    /*  4  */
    sos= sioOutOpen( (void *)los, sioOutLzwPsWriteBytes,
					    sioLzwSeek, sioOutLzwPsClose );

    if  ( ! sos )
	{ XDEB(sos); free( los ); return (SimpleOutputStream *)0; }

    /*  Z  */
    if  ( sioOutLzwPsWriteCode( los, los->losState.lsClearCode ) )
	{ LDEB(1); sioOutClose( sos ); return (SimpleOutputStream *)0; }
